...
export class UpdateCatTreatmentHandler {
  ...
  async handle(treatmentId: string, payload: Partial<TreatmentPayload>, actorUserId: string, isTest: boolean): Promise<{ id: string }> {
    return runInNewTransaction(this.prisma, async (transaction) => {
      const treatment = await transaction.catTreatment.findFirst({ where: { id: treatmentId, deletedAt: null, cat: { isTest } } });
      ...
      await transaction.catTreatment.update({ where: { id: treatmentId }, data: { ...payload, concurrencyToken: crypto.randomUUID() } });
      ...
      if (payload.instructions !== undefined && payload.instructions !== treatment.instructions) changes.push({ eventType: CAT_AUDIT_EVENT_TYPES.treatmentInstructionsChanged, oldValue: treatment.instructions ?? "none", newValue: payload.instructions ?? "none" });
